...
interface FullStatus {
  knowledge?: { findings?: Finding[]; solutions?: Solution[] };
  capabilities?: { learned?: Capability[] };
  [key: string]: unknown;
}
...
export default function KnowledgePage() {
  ...
  return (
    <div style={{ background: "var(--color-surface)" }}>
      <PageHeader icon="◈" title="Knowledge Base" />
          ...
          <>
            {tab === "findings" && (
              <div className="flex flex-col gap-2">
                {findings.length === 0 ? <EmptyState label="findings" /> : (
                  findings.map((f, i) => (
                      ...
                      <div className="flex items-center gap-2 mb-1">
                        {f.source && (
                          <span
                            ...
                            style={{ fontFamily: "'JetBrains Mono', monospace", background: "rgba(255,255,255,0.06)", color: "var(--color-text-secondary)" }}
                          >
                            {String(f.source)}
                          </span>
                        )}
                        {f.timestamp && (
                          <span className="text-[10px] ml-auto" style={{ color: "var(--color-text-muted)", fontFamily: "'JetBrains Mono', monospace" }}>
            ...
            {tab === "solutions" && (
              <div className="flex flex-col gap-2">
                {solutions.length === 0 ? <EmptyState label="solutions" /> : (
                  solutions.map((s, i) => (
                    <div key={i} className="glass-card px-4 py-3">
                      <div className="flex items-start justify-between gap-2 mb-2">
                        ...
                        {s.verified && (
                          <span
                            ...
                            style={{ background: "rgba(0,220,130,0.1)", color: "var(--color-brand)", border: "1px solid rgba(0,220,130,0.2)" }}
                          >
                            verified
                          </span>
                        )}
                      ...
                      {s.solution && (
                        <p
                          ...
                          style={{ fontFamily: "'JetBrains Mono', monospace", background: "rgba(255,255,255,0.03)", color: "var(--color-text-secondary)", border: "1px solid var(--color-border)", wordBreak: "break-word" }}
                        >
                          {String(s.solution)}
                        </p>
                      )}
            ...
            {tab === "capabilities" && (
              <div className="flex flex-col gap-2">
                {capabilities.length === 0 ? <EmptyState label="capabilities" /> : (
                    ...
                    <div key={i} className="glass-card px-4 py-3">
                      ...
                      {c.example && (
                        <p
                          ...
                          style={{ fontFamily: "'JetBrains Mono', monospace", background: "rgba(255,255,255,0.03)", color: "var(--color-text-muted)", border: "1px solid var(--color-border)", wordBreak: "break-all" }}
                        >
                          {String(c.example)}
                        </p>
                      )}
